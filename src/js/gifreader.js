var PixelBuffer = require('./pixelbuffer.js').PixelBuffer;
...
var Stream = function (data) {
    ...
    this.readByte = function () {
        // console.log('readByte', typeof this.data, this.pos)
        if (this.pos >= this.data.length) {
            throw new Error('Attempted to read past end of stream.');
        }
        if (this.data instanceof ArrayBuffer) {
            // console.log('data at ' + this.pos + ': ' + this.data[this.pos])
    ...
    var parseExt = function (block) {
        ...
        var parseAppExt = function (block) {
            ...
            var parseUnknownAppExt = function (block) {
                ...
                // FIvarXME: This won't work if a handler wants to match on any identifier.
...
GifReader.prototype.read = function(url, callback) {
    var _this = this;
    _this.pixelbuffer = new PixelBuffer()
    fetch(url).then(function(response) {
        // console.log('response', response)
        // callback(null)
        response.arrayBuffer().then(function(buf) {
            // console.log('buffer', buf, typeof buf)
            var u8buf = new Uint8Array(buf)

            var st = new Stream(u8buf)
            var last_palette = [];
            var last_delay = 0;
            var last_bg = 0;
            var last_transp = -1;
            ...
            var handler = {
                hdr: function(x) {
                    ...
                    _this.pixelbuffer.init(x.width, x.height, c);
                },
                gce: function(x) {
                ...
                eof: function(x) {
                    _this.pixelbuffer.calc(function(y) {
                        callback(_this);
                    });
                }
            };
